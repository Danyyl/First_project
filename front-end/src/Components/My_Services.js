...
class My_Services extends React.Component{
    constructor(props) {
        super(props);
        this.state = {
            content: [],
        ...
        const answer = this.getMy_Service();
            answer.then(()=>{console.log("My Done")}, ()=>{console.log("My Mistake")});
    }
...
if(prevState.lng !== this.props.lng) {
   ...
   async getMy_Service(){
        ...
        const response = await axios.get("http://localhost:8000/user/service/", config);
        var arr_temp = [];
        for(var i = 0; i < response.data.length; ++i){
            console.log(response.data[i]);
            response.data[i].inform = response.data[i].info;
            arr_temp.push(<My_Service arr = {response.data[i]} func = {this.GetS} lng = {this.props.lng}/>)
        }
